...
record Order(int orderId, List<Product> orderProducts, String status, String phoneNumber, String address) {
    ...
    class Inventory {
        ...
        public void searchByCategory(String categoryName) {
            List<Product> foundProducts = new ArrayList<>();
            String lowerCaseCategoryName = categoryName.toLowerCase(); // Переведення введеної частини назви категорії до нижнього регістру
            boolean categoryFound = false;
            for (Product product : products) {
                String productCategory = product.category().name().toLowerCase(); // Переведення категорії товару до нижнього регістру
                if (productCategory.contains(lowerCaseCategoryName)) { // Перевірка, чи містить категорія товару шукану частину назви
                    foundProducts.add(product);
                    categoryFound = true;
                }
            }
            if (!categoryFound) {
                ...
                return; // Повернення з методу, не виконуючи далі пошук
            }
            if (foundProducts.isEmpty()) {
